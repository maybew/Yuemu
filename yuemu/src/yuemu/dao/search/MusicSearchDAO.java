...
public class MusicSearchDAO {
	...
	/**
	 * 搜索标题内含有指定关键字的音乐资源(用AND连接). 如果传入的集合为空或为null, 返回一个空List.
	 * 
	 * @param titles
	 * @param pageNum
	 * @param pageSize
	 * @return
	 */
	public static List<Music> searchByTitleKeywordsJoiningWithAnd(
			Collection<String> titles, int pageNum, int pageSize) {

		return ResourceSearchDAOTool.searchByTitleKeywordsJoiningWithAnd(
				titles, pageNum, pageSize, Music.class);

	}
	
	/**搜索标签中含有指定关键字的音乐资源(用AND连接). 
