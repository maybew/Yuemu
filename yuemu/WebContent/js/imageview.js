	...
	return {
		css3 : {// css3设置
			...
			show : function() {
				...
				// 设置变形样式
				this._img.style[css3Transform] = "matrix(" + matrix.M11 + "," + matrix.M21 + "," + matrix.M12 + ","
						+ matrix.M22 + ", 0, 0)";
			},
			dispose : function() {
		...
		filter : {// 滤镜设置
			...
			show : function() {
				var img = this._img;
				// 设置滤镜
				$$.extend(img.filters.item("DXImageTransform.Microsoft.Matrix"), getMatrix(this._radian, this._y,
						this._x));
				// 保持居中
...
ImageTrans.prototype._initialize = (function() {
	var init = ImageTrans.prototype._initialize, mousewheel = $$B.firefox ? "DOMMouseScroll" : "mousewheel", methods = {
		"init" : function() {
