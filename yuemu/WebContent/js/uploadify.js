...
SWFUpload.Console.writeLine = function(g) {
	...
	try {
		...
		// TODO Y.L. Wu alert("Exception: " + h.name + " Message: " + h.message);
	}
};
(function(c) {
	var b = {
	...
	var a = {
		...
		onDialogClose : function(d, f, g) {
			...
			if (c.inArray("onDialogClose", e.overrideEvents) < 0) {
				if (this.queueData.filesErrored > 0) {
					// TODO Y.L. Wu alert(this.queueData.errorMsg);
				}
			}
			if (e.onDialogClose) {
		...
		onSelect : function(g) {
			...
			for ( var f in this.queueData.files) {
				...
				if (e.uploaded != true && e.name == g.name) {
					// TODO Y.L. Wu var d = confirm('The file named "'
							//+ g.name
		...
		onUploadError : function(e, i, h) {
			...
			case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
				// TODO Y.L. Wu alert("The upload limit has been reached (" + h + ").");
